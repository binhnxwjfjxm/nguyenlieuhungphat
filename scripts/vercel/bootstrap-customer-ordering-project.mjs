...
async function getProject({ apiBase, teamId, token, fetchImpl }) {
  ...
  if (!response.ok) throw new Error(`Unable to read Vercel project (${response.status}): ${body?.error?.message ?? body?.message ?? "unknown error"}`);
  return body;
}
...
async function createProject({ apiBase, teamId, token, fetchImpl }) {
  ...
  for (const version of ["v10", "v9"]) {
    ...
    throw new Error(`Unable to create Vercel project (${response.status}): ${body?.error?.message ?? body?.message ?? "unknown error"}`);
  }
...
function writeGithubOutput(projectId) {
