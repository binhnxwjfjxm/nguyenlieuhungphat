...
import { type QuoteRequestInput, sanitizeText, validateQuoteInput } from "@/lib/validation";
...
const EMPTY: ContactState = { name: "", phone: "", company: "", email: "", note: "", honeypot: "" };
...
export function ContactForm() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const payload: QuoteRequestInput = {
      ...
      area: "khac",
      usage: "khac",
      note: sanitizeText(form.note, 500),
    ...
    const validated = validateQuoteInput(payload);
    if (!validated.ok) {
    ...
    try {
      ...
      const result = (await response.json()) as { ok?: boolean; code?: string; retryAfter?: number };
    ...
    <form className="contact-form-v2" onSubmit={handleSubmit}>
      ...
      <div className="form-grid">
        <label className="field">
          <span>Họ tên *</span>
          <input autoComplete="name" required value={form.name} onChange={(event) => update("name", event.target.value)} placeholder="Nguyễn Văn A" />
          {errors.name ? <em>{errors.name}</em> : null}
        </label>

        <label className="field">
          <span>Số điện thoại *</span>
          <input autoComplete="tel" inputMode="tel" required value={form.phone} onChange={(event) => update("phone", event.target.value)} placeholder="0912345678" />
          {errors.phone ? <em>{errors.phone}</em> : null}
        </label>

        <label className="field field-wide">
          <span>Công Ty / cửa hàng</span>
          <input autoComplete="organization" value={form.company} onChange={(event) => update("company", event.target.value)} placeholder="Tên đơn vị" />
          {errors.company ? <em>{errors.company}</em> : null}
        </label>

        <label className="field field-wide">
          <span>Email</span>
          <input autoComplete="email" type="email" value={form.email} onChange={(event) => update("email", event.target.value)} placeholder="email@congty.com" />
          {errors.email ? <em>{errors.email}</em> : null}
        </label>

        <label className="field field-wide">
          <span>Nội dung liên hệ *</span>
          <textarea required rows={5} value={form.note} onChange={(event) => update("note", event.target.value)} placeholder="Anh/chị muốn trao đổi nội dung gì với Hưng Phát?" />
          {errors.note ? <em>{errors.note}</em> : null}
        </label>

        <label className="contact-honeypot" aria-hidden="true">
          <span>Website</span>
          <input tabIndex={-1} autoComplete="off" value={form.honeypot} onChange={(event) => update("honeypot", event.target.value)} />
        </label>
      </div>
