...
function loadServiceAccount(): ServiceAccount {
  ...
  console.info("ordering_dialogflow_cx_runtime_identity", {
...
async function getAccessToken(serviceAccount: ServiceAccount) {
  const auth = new GoogleAuth({ credentials: serviceAccount, scopes: [DIALOGFLOW_SCOPE] });
...
function requestClass(result: Record<string, unknown>): "flow" | "playbook" {
  ...
  return generativeInfo && typeof generativeInfo === "object" && !Array.isArray(generativeInfo) ? "playbook" : "flow";
}

export async function detectDialogflowReply(input: { sessionId: string; message: string }) {
  ...
  const response = await fetch(`${baseUrl}/${sessionPath}:detectIntent`, {
    ...
    body: JSON.stringify({
      queryInput: { languageCode: agent.languageCode, text: { text: input.message.trim() } },
  ...
  if (!response.ok) {
    ...
    const providerError = providerPayload?.error && typeof providerPayload.error === "object" && !Array.isArray(providerPayload.error)
      ...
      : {};
    console.error("ordering_dialogflow_cx_detect_intent_provider_error", {
