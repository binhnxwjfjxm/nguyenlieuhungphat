...
export function QuickOrder() {
  ...
  async function addSelectedToCart() {
    ...
    for (const [productId, quantity] of selectedEntries) {
      const existingIndex = nextLines.findIndex((line) => line.productId === productId);
      if (existingIndex >= 0) {
        ...
        nextLines.push({ productId, quantity });
      }
    }
  ...
  return (
    <section className="quick-order-screen">
      <div className="quick-order-intro">
        <div>
          <p className="eyebrow">Nhập số lượng thật nhanh</p>
          <h1>Đặt hàng nhanh</h1>
          <p>Không cần mở ảnh; chọn nhiều mặt hàng rồi thêm vào giỏ một lần.</p>
        </div>
        <Link className="quick-cart-link" href="/cart">
          <ShoppingCart aria-hidden="true" size={18} />
          Xem giỏ
        </Link>
      </div>

      <label className="catalog-search quick-order-search">
      ...
      <div className="quick-filter-row">
        <div aria-label="Lọc danh mục đặt nhanh" className="catalog-category-row" role="tablist">
          <button
        ...
        <button
          ...
          className={`selected-only-button ${selectedOnly ? "is-active" : ""}`}
          onClick={() => setSelectedOnly((current) => !current)}
          type="button"
        >
          <Check aria-hidden="true" size={16} />
          Đã chọn ({selectedLines})
        </button>
      </div>
      ...
      {error ? (
        ...
        <div aria-label="Đang tải danh sách đặt nhanh" className="quick-order-list">
          {Array.from({ length: 5 }, (_, index) => (
            <div className="quick-order-row is-skeleton" key={index} />
          ))}
        </div>
      ) : visibleProducts.length === 0 ? (
        <div className="catalog-state-card">
          <PackageSearch aria-hidden="true" size={30} />
          <strong>{selectedOnly ? "Chưa chọn sản phẩm nào" : "Không tìm thấy sản phẩm"}</strong>
          <span>
            {selectedOnly ? "Nhập số lượng ở danh sách trước khi bật bộ lọc này." : "Thử đổi từ khóa hoặc danh mục."}
          </span>
        </div>
      ) : (
        <div className="quick-order-list">
          {visibleProducts.map((product) => {
            ...
            return (
              <article className={`quick-order-row ${canOrder ? "" : "is-disabled"}`} key={product.id}>
                <Link className="quick-product-copy" href={`/products/${product.id}`}>
                ...
                <div className="quick-quantity-control" aria-label={`Số lượng ${product.name}`}>
                  ...
                  <div className="quick-step-buttons">
                    <button
                      ...
                      disabled={!canOrder || quantity >= 999}
                      onClick={() => changeQuantity(product, quantity + 1)}
                      type="button"
                    >
                  ...
                  <button
                    aria-label={`Tăng nhanh ${product.name}`}
                    className="quick-plus-button"
                    disabled={!canOrder || quantity >= 999}
                    onClick={() => changeQuantity(product, quantity + 1)}
                    type="button"
                  >
      ...
      <div className="quick-order-summary" aria-live="polite">
        <div>
          <span>{selectedLines} dòng đã chọn</span>
          <strong>{selectedQuantity} sản phẩm</strong>
        </div>
        <button disabled={selectedLines === 0} onClick={() => void addSelectedToCart()} type="button">
