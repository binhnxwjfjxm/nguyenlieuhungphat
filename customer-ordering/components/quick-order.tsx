...
export function QuickOrder() {
  ...
  return <section className="quick-order-screen quick-order-screen-compact">
    ...
    <label className="catalog-search quick-order-search"><Search aria-hidden="true" size={19} /><span className="sr-only">Tìm sản phẩm đặt nhanh</span><input autoComplete="off" onChange={(event) => setQuery(event.target.value)} placeholder="Tên hoặc SKU" type="search" value={query} />{query ? <button aria-label="Xóa nội dung tìm kiếm" onClick={() => setQuery("")} type="button"><RotateCcw aria-hidden="true" size={17} /></button> : null}</label>
