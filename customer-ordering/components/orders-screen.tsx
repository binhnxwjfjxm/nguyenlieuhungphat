...
export function OrdersScreen() {
  ...
  return <section className="orders-screen orders-screen-compact">
    ...
    <label className="orders-search"><Search aria-hidden="true" size={19} /><span className="sr-only">Tìm kiếm</span><input onChange={(event) => setQuery(event.target.value)} placeholder={view === "orders" ? "Mã đơn, SKU hoặc sản phẩm" : "Tên hoặc SKU đã mua"} type="search" value={query} /></label>

    {view === "orders" ? <>
      {orders.length > 0 ? <div aria-label="Lọc theo trạng thái" className="orders-status-filters" role="group">{ORDER_STATUS_FILTERS.map((filter) => <button className={statusFilter === filter.value ? "is-active" : ""} key={filter.value} onClick={() => setStatusFilter(filter.value)} type="button">{filter.label}</button>)}</div> : null}
