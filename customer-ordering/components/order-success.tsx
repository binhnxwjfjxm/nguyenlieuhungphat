...
export function OrderSuccess({ orderId }: Readonly<{ orderId: string }>) {
  ...
  if (!loaded) {
    return (
      <section aria-label="Đang tải kết quả đặt hàng" className="order-success-screen is-loading">
  ...
  if (!order) {
    return (
      <section className="cart-empty-screen">
        ...
        <h1>Không tìm thấy đơn vừa gửi</h1>
        <p>Dữ liệu mock có thể đã bị xóa trên trình duyệt này.</p>
  ...
  return (
    <section className="order-success-screen">
      <span className="success-icon-shell">
        <CheckCircle2 aria-hidden="true" size={44} />
      </span>
      <p className="eyebrow">Đã gửi thành công</p>
      <h1>Hưng Phát đã nhận yêu cầu</h1>
      <p className="success-lead">
        Đơn mock đã được lưu trên thiết bị. Khi nối Core, trạng thái tiếp nhận sẽ do hệ thống trả về.
      </p>
      ...
      <section className="success-order-card">
        ...
        <dl>
          ...
          <div>
            <dt>Tạm tính có giá</dt>
            <dd>{formatMoney(order.pricedSubtotal)}</dd>
          </div>
        </dl>
        {order.hasPendingPrice ? (
          <p>Có sản phẩm chờ Hưng Phát xác nhận giá.</p>
        ) : null}
      </section>

      <section className="success-address-card">
        <MapPin aria-hidden="true" size={20} />
        <div>
          <strong>{order.address.label}</strong>
          <span>{order.address.recipientName} · {order.address.phone}</span>
