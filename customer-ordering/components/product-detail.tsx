...
export function ProductDetail({ sku }: Readonly<{ sku: string }>) {
  ...
  if (!product) return <section className="catalog-state-card product-not-found"><PackageSearch aria-hidden="true" size={32} /><strong>Không tìm thấy sản phẩm</strong><span>Sản phẩm này không còn trong danh mục. Hãy quay lại để tìm mặt hàng khác.</span><Link className="secondary-link-button" href="/products"><ArrowLeft aria-hidden="true" size={17} /> Về danh mục</Link></section>;
  ...
  return <article className="product-detail-screen">
    ...
    <div className="product-detail-copy">
      <div className="catalog-product-meta"><span>{purchaseModeLabel(product)}</span><span className={`availability-${product.availability}`}>{availabilityText(product)}</span></div>
      ...
      <dl className="product-spec-grid product-spec-grid-expanded">
        <div><dt>Quy cách mua</dt><dd>{purchaseModeLabel(product)}</dd></div><div><dt>Quy cách</dt><dd>{product.packaging}</dd></div><div><dt>Thương hiệu</dt><dd>{product.brand}</dd></div><div><dt>Loại</dt><dd>{product.productType}</dd></div><div><dt>Vị</dt><dd>{product.flavor ?? "Không áp dụng"}</dd></div><div><dt>Size</dt><dd>{product.size || "—"}</dd></div>
      </dl>
      <div className="product-price-panel"><span>{product.purchaseMode === "case" ? "Giá thùng" : "Giá lẻ"}</span><strong className={product.price.status === "available" ? "" : "is-pending"}>{formatPrice(product)}</strong>{product.price.status !== "available" ? <small>Hưng Phát sẽ xác nhận giá chính thức trước khi xử lý đơn.</small> : null}{product.purchaseMode === "case" ? <small>Giá thùng áp dụng theo đúng quy cách thùng đã chọn.</small> : null}</div>
