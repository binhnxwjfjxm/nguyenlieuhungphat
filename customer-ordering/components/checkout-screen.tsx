...
export function CheckoutScreen() {
  ...
  if (error && !cart) return <section className="catalog-state-card cart-state-card is-error" role="alert"><PackageCheck aria-hidden="true" size={30} /><strong>Chưa tải được checkout</strong><span>{error}</span></section>;
  ...
  return <section className="checkout-screen">
    ...
    <section className="checkout-total-card"><div><span>Tạm tính các dòng có giá</span><strong>{formatMoney(pricedSubtotal)}</strong></div><p>{hasPendingPrice ? "Đơn có sản phẩm chưa có giá. Hưng Phát sẽ xác nhận giá chính thức khi tiếp nhận." : "Giá được lấy theo đúng SKU từ dữ liệu bảng giá."}</p>{error ? <div className="checkout-error" role="alert">{error}</div> : null}<button disabled={submitting || !addressId} onClick={() => void submit()} type="button">{submitting ? <LoaderCircle aria-hidden="true" className="is-spinning" size={20} /> : <PackageCheck aria-hidden="true" size={20} />}{submitting ? "Đang gửi đơn..." : "Gửi đơn hàng"}</button></section>
  </section>;
}
