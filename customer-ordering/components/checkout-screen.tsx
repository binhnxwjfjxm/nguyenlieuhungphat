...
function newSubmissionKey(): string {
  ...
  return `mock-submit-${Date.now()}-${Math.random().toString(36).slice(2)}`;
}
...
export function CheckoutScreen() {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      service.listDeliveryAddresses(),
      service.getCheckoutDraft(),
    ])
      ...
      .catch(() => {
        if (!cancelled) {
          setError("Không tải được thông tin xác nhận đơn.");
          setLoaded(true);
        }
      });
    return () => {
  ...
  const lines = cart?.lines.map((line) => ({ line, product: productMap.get(line.productId) })) ?? [];
  const totalQuantity = lines.reduce((total, item) => total + item.line.quantity, 0);
  const pricedSubtotal = lines.reduce((total, item) => {
    const amount =
  ...
  const hasPendingPrice = lines.some(
    (item) => item.product?.price.status !== "available" || item.product.price.amount === null,
  );

  async function saveDraft(nextAddressId: string | null, nextNote: string) {
    const draft: CheckoutDraft = {
      addressId: nextAddressId,
      orderNote: nextNote,
      updatedAt: new Date().toISOString(),
    };
    await service.saveCheckoutDraft(draft);
  }
  ...
  async function submit() {
    ...
    try {
      ...
      setError(reason instanceof Error ? reason.message : "Không gửi được đơn hàng.");
  ...
  if (!loaded) {
    return (
      <section aria-label="Đang tải xác nhận đơn" className="checkout-screen">
        <div className="checkout-card is-skeleton" />
        <div className="checkout-card is-skeleton" />
      </section>
    );
  }
  ...
  if (error && !cart) {
    return (
      <section className="catalog-state-card cart-state-card is-error" role="alert">
        ...
        <strong>Chưa tải được checkout</strong>
        <span>{error}</span>
      </section>
    );
  }
  ...
  if (!cart || cart.lines.length === 0) {
    return (
      <section className="cart-empty-screen">
        ...
        <p className="eyebrow">Xác nhận đơn</p>
        <h1>Giỏ hàng đang trống</h1>
        <p>Hãy thêm ít nhất một sản phẩm trước khi gửi đơn.</p>
  ...
  return (
    <section className="checkout-screen">
      <Link className="product-back-link" href="/cart">
      ...
      <div className="checkout-heading">
        ...
        <p>Kiểm tra địa chỉ, ghi chú và số lượng trước khi gửi.</p>
      </div>
      ...
      <section className="checkout-card">
        <div className="checkout-card-title">
          ...
          <div>
            ...
            <span>Chọn một địa chỉ đã lưu</span>
          </div>
        </div>
        <div className="checkout-address-list">
          {addresses.map((address) => (
            <label className={`checkout-address ${addressId === address.id ? "is-selected" : ""}`} key={address.id}>
              <input
                checked={addressId === address.id}
                name="delivery-address"
                onChange={() => {
                  setAddressId(address.id);
                  void saveDraft(address.id, orderNote);
                }}
                type="radio"
                value={address.id}
              />
              ...
              <span>
                <strong>
                  {address.label}
                  {address.isDefault ? <small>Mặc định</small> : null}
                </strong>
                <b>{address.recipientName} · {address.phone}</b>
                <em>{address.addressLine}</em>
              </span>
            </label>
          ))}
        </div>
      ...
      <section className="checkout-card">
        <div className="checkout-card-title">
          ...
          <div>
            ...
            <span>{lines.length} dòng · {totalQuantity} sản phẩm</span>
          </div>
        </div>
          ...
          {lines.map(({ line, product }) => (
            <div className="checkout-line" key={line.productId}>
              <span>
                ...
                <small>{line.quantity} {product?.unit ?? "đơn vị"}{line.note ? ` · ${line.note}` : ""}</small>
              </span>
              <b>
                {product?.price.status === "available" && product.price.amount !== null
                  ? formatMoney(product.price.amount * line.quantity)
                  : "Chờ giá"}
              </b>
            </div>
          ))}
      ...
      <label className="checkout-card checkout-note-card">
        <span>Ghi chú toàn đơn</span>
        <textarea
          maxLength={500}
          onBlur={() => void saveDraft(addressId, orderNote)}
      ...
      <section className="checkout-total-card">
        ...
        <p>
          {hasPendingPrice
            ? "Đơn có sản phẩm chưa có giá. Hưng Phát sẽ xác nhận giá chính thức khi tiếp nhận."
            : "Giá hiện tại là dữ liệu mock và chưa phải báo giá chính thức."}
        </p>
