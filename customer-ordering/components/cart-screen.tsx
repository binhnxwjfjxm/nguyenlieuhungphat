    ...
    <div className="cart-line-list">{displayLines.map(({ line, product }) => {
      ...
      return <article className="cart-line-row" key={line.sku}>
        <div className="cart-line-primary">
          <div className="cart-product-copy"><span>{line.sku}</span><h2>{product?.name ?? "Sản phẩm không còn"}</h2><p>{product ? `${product.packaging} · ${product.unit}` : "Không tìm thấy thông tin sản phẩm"}</p></div>
          <button aria-label={`Xóa ${product?.name ?? line.sku}`} className="cart-remove-button" onClick={() => void persist(cart.lines.filter((item) => item.sku !== line.sku))} type="button"><Trash2 aria-hidden="true" size={15} /></button>
        </div>
        <div className="cart-line-actions">
          {familyVariants.length > 1 ? <div className="cart-variant-switch" role="group" aria-label={`Chọn mua lẻ hoặc thùng cho ${product?.name ?? line.sku}`}>{familyVariants.map((variant) => <button aria-pressed={variant.sku === line.sku} className={variant.sku === line.sku ? "is-active" : ""} key={variant.sku} onClick={() => void switchVariant(line.sku, variant.sku)} type="button">{purchaseModeLabel(variant)}</button>)}</div> : <span className="cart-mode-static">{product ? purchaseModeLabel(product) : "—"}</span>}
          <div className="quantity-stepper cart-quantity-stepper" aria-label={`Số lượng ${product?.name ?? line.sku}`}><button aria-label="Giảm số lượng" disabled={line.quantity <= 1} onClick={() => void persist(cart.lines.map((item) => item.sku === line.sku ? { ...item, quantity: Math.max(1, item.quantity - 1) } : item))} type="button"><Minus aria-hidden="true" size={14} /></button><input aria-label={`Nhập số lượng ${product?.name ?? line.sku}`} inputMode="numeric" max={999} min={1} onChange={(event) => { const next = Math.min(999, Math.max(1, Number(event.target.value) || 1)); void persist(cart.lines.map((item) => item.sku === line.sku ? { ...item, quantity: next } : item)); }} onFocus={(event) => event.currentTarget.select()} type="number" value={line.quantity} /><button aria-label="Tăng số lượng" disabled={line.quantity >= 999} onClick={() => void persist(cart.lines.map((item) => item.sku === line.sku ? { ...item, quantity: Math.min(999, item.quantity + 1) } : item))} type="button"><Plus aria-hidden="true" size={14} /></button></div>
        ...
        <label className="cart-line-note cart-line-note-compact"><span>Ghi chú cho {product?.name ?? line.sku}</span><input maxLength={180} onBlur={() => void persistCurrentCart()} onChange={(event) => updateLocalNote(line.sku, event.target.value)} placeholder="Ghi chú cho mặt hàng" value={line.note ?? ""} /></label>
      </article>;
    })}</div>
