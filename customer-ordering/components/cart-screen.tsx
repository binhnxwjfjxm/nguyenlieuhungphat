...
export function CartScreen() {
  ...
  if (displayLines.length === 0) return <section className="cart-empty-screen"><span className="cart-empty-icon"><ShoppingBasket aria-hidden="true" size={34} /></span><p className="eyebrow">Giỏ hàng</p><h1>Chưa có sản phẩm</h1><p>Chọn từ danh mục hoặc dùng màn Đặt nhanh để nhập nhiều mặt hàng.</p><div className="cart-empty-actions"><Link className="primary-link-button" href="/quick-order">Đặt hàng nhanh</Link><Link className="secondary-link-button" href="/products">Xem sản phẩm</Link></div></section>;

  return <section className="cart-screen">
    <div className="cart-heading"><div><p className="eyebrow">Kiểm tra trước khi đặt</p><h1>Giỏ hàng của bạn</h1><p>{displayLines.length} dòng · {totalQuantity} sản phẩm</p></div><button className="cart-clear-button" onClick={() => void persist([])} type="button"><Trash2 aria-hidden="true" size={16} />Xóa tất cả</button></div>
    <div className="cart-line-list">{displayLines.map(({ line, product }) => {
      ...
      return <article className="cart-line-card" key={line.sku}><div className="cart-line-top"><Link className="cart-product-copy" href={`/products/${encodeURIComponent(line.sku)}`}><span>{line.sku}</span><h2>{product?.name ?? "Sản phẩm không còn"}</h2><p>{product ? `${product.packaging} · ${product.unit}` : "Không tìm thấy thông tin sản phẩm"}</p></Link><button aria-label={`Xóa ${product?.name ?? line.sku}`} className="cart-remove-button" onClick={() => void persist(cart.lines.filter((item) => item.sku !== line.sku))} type="button"><Trash2 aria-hidden="true" size={17} /></button></div>
        ...
        <label className="cart-line-note"><span>Ghi chú mặt hàng</span><input maxLength={180} onBlur={() => void persistCurrentCart()} onChange={(event) => updateLocalNote(line.sku, event.target.value)} placeholder="Ví dụ: giao nguyên thùng, gọi trước..." value={line.note ?? ""} /></label></article>;
    })}</div>
    <div className="cart-summary-card"><div><span>Tạm tính các dòng có giá</span><strong>{formatMoney(pricedSubtotal)}</strong></div>{pendingPriceCount > 0 ? <p>{pendingPriceCount} dòng chưa có giá sẽ được Hưng Phát xác nhận sau.</p> : <p>Giá hiển thị lấy theo đúng SKU trong bảng giá.</p>}<Link className="cart-checkout-button" href="/checkout">Tiếp tục xác nhận đơn<ArrowRight aria-hidden="true" size={18} /></Link></div>
  </section>;
}
