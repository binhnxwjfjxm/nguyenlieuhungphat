...
const aliases={
  ...
  category:['nhom chinh','nganh hang','nganh','nhom san pham','nhom sp','phan nhom','nhom hang','nhom'],
  ...
  packaging:['quy cach dong goi','quy cach le','quy cach','dong goi','packaging'],casePackaging:['quy cach thung','dong goi thung','quy cach si'],
  unit:['don vi tinh','dvt','unit','don vi'],caseQuantity:['sl thung','so luong thung','quy doi thung','sl/thung','cay thung'],
};
...
function skuLike(v){const x=clean(v),n=norm(x);if(!x||x.length>48||/^(stt|sku|ma|ma hang|ma san pham|tong|total)$/.test(n)||/^(\d{1,2}[/-]){2}\d{2,4}$/.test(x))return false;if(/\s/.test(x)&&x.split(/\s+/).length>2)return false;return /^[\p{L}0-9._/+\-]+$/u.test(x)&&(/[0-9]/.test(x)||(x.length<=24&&/[A-Z]/.test(x)));}
...
function infer(rs){
  ...
  for(let sc=0;sc<w;sc++)for(let nc=0;nc<w;nc++){if(sc===nc)continue;let matched=0,first=-1,cases=0;const vals=new Set();
    for(let i=0;i<lim;i++){const s=sku(rs[i][sc]??''),n=clean(rs[i][nc]??'');if(!skuLike(s)||!nameLike(n))continue;if(first<0)first=i;matched++;vals.add(s);if(s.endsWith('T'))cases++;}
...
function header(rs){let best=null,lim=Math.min(rs.length,250);for(let i=0;i<lim;i++)for(const span of [1,2,3]){if(i+span>lim)continue;const m=mapHeaders(mergedHeader(rs,i,span));if(m.columns.sku===undefined||m.columns.name===undefined)continue;const x={dataStart:i+span,columns:m.columns,mode:span===1?'header':'multi-row-header',score:m.score+(span===1?2:0)};if(!best||x.score>best.score)best=x;}return best??infer(rs);}
